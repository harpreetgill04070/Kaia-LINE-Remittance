...
class ContractIntegration {
  ...
  initializeContracts() {
    try {
      ...
      if (routerAddress) {
        this.remittanceRouter = new ethers.Contract(routerAddress, REMITTANCE_ROUTER_ABI, this.provider);
        console.log('✅ RemittanceRouter contract initialized:', routerAddress);
      }
  ...
  initializeTokenContracts() {
    ...
    for (const [symbol, address] of Object.entries(tokenAddresses)) {
      if (address) {
        this.tokenContracts[symbol] = new ethers.Contract(address, ERC20_ABI, this.provider);
  ...
  async getTokenBalance(userAddress, tokenSymbol) {
    try {
      ...
      console.error(`❌ Failed to get ${tokenSymbol} balance:`, error.message);
      return '0';
    }
  }
  ...
  async createRemittanceTransaction(intent, userAddress) {
    try {
      ...
      if (!isAllowed) {
        throw new Error(`Token ${token} is not allowed for remittances`);
      }
      ...
      console.error('❌ Failed to create remittance transaction:', error.message);
      return {
        success: false,
        error: error.message
      };
